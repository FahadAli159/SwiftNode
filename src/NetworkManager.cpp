...
NetworkManager::NetworkManager(int port)
    : myPort(port), running(false), serverSocket(-1) {}
...
void NetworkManager::acceptLoop() {
  while (running) {
    ...
    std::lock_guard<std::mutex> lock(threadsMutex);
    clientThreads.emplace_back(&NetworkManager::handleClient, this,
                               clientSocket);
  }
}
...
void NetworkManager::sendData(const std::string &ip, int port,
